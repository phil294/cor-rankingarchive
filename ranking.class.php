...
class Ranking
{
    public static $REALMS = [2012 => 'Alsius', 4 => 'Ignis', 3 => 'Syrtis']; // values just for orientation
    public static $CLASSES = [1 => 'Warrior', 2 => 'Archer', 3 => 'Mage'];
    ...
    /** for date=today */
    public function insert_ignore_krp($player_id, $rlmp)
    {
        ...
        $today = date('Y-m-d'); // für -12h: 2. param ,time()-43200);
        $sql = "insert ignore into krp (player, date, rlmp) values ($player_id, \"" . $today . "\", $rlmp)"; // SILENTLY fails if already in existence
        $this->query($sql);
    }
    ...
    public function get_rlmps($name, $realm)
    {
        ...
        $rlmps = [];
        $sql = "SELECT rlmp,date FROM krp LEFT JOIN player on krp.player = player.id WHERE name = \"$name\" and realm = $realm";
        $result = $this->query($sql);
        $last_time = 0;
        while ($row = mysqli_fetch_object($result)) {
